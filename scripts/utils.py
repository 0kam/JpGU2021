...
# Set up patches
def read_sses(label_dir, image_size, label="all", shrink=0):
    """
...
def set_patches(label_dir, image_dir, out_dir, kernel_size, batch_size=50, label="all", unlabelled=False, shrink=0):
    ...
    mask, labels_list = read_sses(label_dir, image.size, label=label, shrink=shrink)
...
# Drawing functions
def draw_teacher(out_path, label_dir, class_to_idx, image_size, label, shrink):
    img, labels = read_sses(label_dir, image_size, label=label, shrink=shrink)
    mask = img.copy()
...
def draw_legend(out_path, label_dir, class_to_idx, label):
    img, labels = read_sses(label_dir, (9999,9999), label=label)
